...
public class FMGA1 {
	...
	public void Input(int motifLength) {
		...
		Candidate[] can=new  Candidate[10];
		int ml = motifLength;
		int n = 0;
		...
		try {
			...
			for (int i1 = 0; i1 < pop_size; i1++) {
				...
				temp[0] = (seq[i1].substring(t, t + ml));// v vll make a Candidate array named-Initials and best candidate motif
				temp[1] = seq[i1].substring(m, m + ml);				// in these 4 Candidate motifs vl be chosen for can
