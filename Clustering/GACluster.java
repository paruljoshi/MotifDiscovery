...
public class GACluster{
	int motifLength;
	int nm;
	int mut_per;
	Candidate[] can; //objects initialized to use methods of Candidate Class
	Candidate[] Initial;
	Candidate[] pool;
	Candidate[] mate_pool;
	Candidate[] next_can;
	...
	String[] parent12;
	String[] child12;
	char[] Base={'A','T','G','C'}; //posssible inputs
	...
	int z=0;
	 ...
	 ArrayList<Candidate> cl=new ArrayList<Candidate>();
		...
		try {
			...
			temp = in.next(); //to get each sequence from input file
			while (in.hasNext()) {
				String chromosome = "";
				geneName.add(temp);
...
//Now printing each sequence with its Name and Code
			...
			for (int i = 0; i < chromosomes.size(); i++){
				...
				for (int j = 0; j < nm; j++) {
					randomindex[j] = r.nextInt(range - ml); //random positions are stored in randomindex[j]
				}
				System.out.println(geneName.get(i));
				for (int j = 0; j < nm; j++){
					String s1=chromosomes.get(i).substring(randomindex[j],randomindex[j] + ml); //to get random gene acc. to position generated
					Candidate c=new Candidate(s1);
					c.setPosition(randomindex[j]);
					Initial[j]=c;
					System.out.println(Initial[j].getName()+"   "+Initial[j].getPosition());
				}
				//the Initial ARRAY FORMED OUT OF EVERY SEQUENCE IS THEN SEND TO THE FITNESS FUNCTION 
	...
	public Candidate[] FitnessCalculation(Candidate[] c){
		int count=c.length;		//count is the total random motifs generated in a sequence
		float fit=0;
		float f=0;
...
//to get the total count of A,T,G,C in each column
